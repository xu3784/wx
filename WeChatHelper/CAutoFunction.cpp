...
void OutputExpression(DWORD dwExpressionAddr)
{
	...
	//创建文件
	HANDLE hFile = CreateFileA(imagepath, GENERIC_ALL, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
	...
	//写入文件
	DWORD dwWrite = 0;
	WriteFile(hFile, (LPCVOID)dwImage, dwImageLen, &dwWrite, nullptr);
	//关闭句柄
	CloseHandle(hFile);
}
