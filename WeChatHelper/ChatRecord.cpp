...
void HookChatRecord()
{
	...
	*(DWORD*)&bJmpCode[1] = (DWORD)RecieveWxMesage - dwHookAddr - 5;
	...
	ReadProcessMemory(GetCurrentProcess(), (LPVOID)dwHookAddr, originalCode, 5, nullptr);

	//覆盖指令 B9 E8CF895C //mov ecx,0x5C89CFE8
	WriteProcessMemory(GetCurrentProcess(), (LPVOID)dwHookAddr, bJmpCode, 5, nullptr);
}
...
void SendWxMessage()
{
	...
	if (hWnd == nullptr)
	{
		MessageBoxA(nullptr, "未找到微信助手窗口", "发送端", MB_OK);
	}
