...
void InitWindow(HMODULE hModule)
{
	...
	if (IsWxVersionValid())
	{
		...
		{
			//开线程持续检测微信登陆状态
			CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)CheckIsLogin, nullptr, 0, nullptr);
		...
		else
		{
			//如果微信已经登陆 发送消息给客户端
			//查找登陆窗口句柄
			HWND hLogin = FindWindow(nullptr, L"Login");
			if (hLogin == nullptr)
			{
				MessageBoxA(nullptr, "未查找到Login窗口", "错误", MB_OK);
				return;
			}
			COPYDATASTRUCT login_msg;
			login_msg.dwData = WM_AlreadyLogin;
			login_msg.lpData = nullptr;
			login_msg.cbData = 0;
			//发送消息给控制端
	...
	{
		MessageBoxA(nullptr, "当前微信版本不匹配，请下载WeChat2.6.8.52", "错误", MB_OK);
	}
}
