...
void ShowChatRoomUser(wchar_t* chatroomwxid)
{
	...
	//完成之后发送消息
	HWND hWnd = FindWindow(nullptr, TEXT("ChatRoomMember"));
	if (hWnd == nullptr)
	{
		MessageBoxA(nullptr, "未找到ChatRoomMember窗口", "错误", 0);
		return;
	}
	...
	userinfodata.lpData = nullptr; // szSendBuf;//待发送的数据的起始地址(可以为NULL)
...
void DelRoomMember(wchar_t* roomid, wchar_t* memberwxid)
{
	...
	MessageBoxA(nullptr, tempbuff, "地址", 0);
