...
void SaveImg(DWORD qrcode)
{
	...
	//将文件写到Temp目录下
	HANDLE hFile = CreateFileA(szPicturePath, GENERIC_ALL, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
	if (hFile == nullptr)
	{
		MessageBoxA(nullptr, "创建图片文件失败", "错误", 0);
		return;
	}

	DWORD dwRead = 0;
	if (WriteFile(hFile, PicData, cpyLen, &dwRead, nullptr) == 0)
	{
		MessageBoxA(nullptr, "写入图片文件失败", "错误", 0);
		return;
	}
	CloseHandle(hFile);
