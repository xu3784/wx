...
void ReSendUser(UserInfo* user)
{
	...
	{
		//发送到控制端
		hWnd = FindWindow(nullptr, TEXT("微信助手"));
		if (hWnd == nullptr)
		{
			continue;
		}
...
void SendUserListInfo()
{
	...
	//发送到控制端
	HWND hWnd = FindWindow(nullptr, TEXT("微信助手"));
	if (hWnd == nullptr)
	{
		//创建线程 发送消息 防止漏掉联系人
		CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)ReSendUser, user, 0, nullptr);
	}
...
void SendFileMessage(wchar_t* wxid, wchar_t* filepath)
{
	...
	filePath filePathStruct = {nullptr};
...
void SendXmlCard(wchar_t* RecverWxid, wchar_t* SendWxid, wchar_t* NickName)
{
	...
	wxString pWxid = {nullptr};
	wxString pXml = {nullptr};
