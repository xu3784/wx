...
int PatchWeChat()
{
	...
	pbuffer = VirtualAlloc(nullptr, 0x1000, MEM_COMMIT, PAGE_READWRITE);
	...
	if (!NT_SUCCESS(Status))
	{
		...
		pbuffer = VirtualAlloc(nullptr, dwSize * 2, MEM_COMMIT, PAGE_READWRITE);
		...
		Status = ZwQuerySystemInformation(SystemHandleInformation, pbuffer, dwSize * 2, nullptr);
...
void OpenWeChat()
{
	...
	if (ERROR_SUCCESS != RegQueryValueEx(hKey, L"InstallPath", nullptr, &Type, (LPBYTE)Path, &cbData))
	{
		goto __exit;
	}
