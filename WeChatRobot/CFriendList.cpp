...
void CFriendList::OnRclickFriendlist(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	//判断选中的是群ID还是个人微信ID
	if (StrStrW(wxid, L"chatroom") == nullptr)
	{
		//弹出好友相关的菜单
		CMenu m_Menu, *tMenu;
		...
		//弹出菜单
		TrackPopupMenu(tMenu->m_hMenu, TPM_LEFTALIGN, pt.x, pt.y, 0, m_hWnd, nullptr);
	}
	else
	{
		//弹出群相关的菜单
...
{
	if (MessageBoxA(nullptr, "是否删除好友", "Tip", MB_YESNO))
	{
		if (wcscmp(wxid, L"filehelper") == 0)
		{
			MessageBoxA(nullptr, "无法删除filehelper", "Tip", MB_OK);
			return;
		}
		//查找窗口
		CWnd* pWnd = CWnd::FindWindow(nullptr, L"WeChatHelper");
...
{
	if (MessageBoxA(nullptr, "是否退出群聊", "Tip", MB_YESNO))
	{
		//查找窗口
		CWnd* pWnd = CWnd::FindWindow(nullptr, L"WeChatHelper");
