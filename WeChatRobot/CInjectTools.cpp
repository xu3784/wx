...
BOOL InjectDll()
{
	...
	{
		MessageBoxA(nullptr, "获取当前工作目录失败", "错误", 0);
		return FALSE;
	}
	...
	if (dwPid == 0)
	{
		MessageBoxA(nullptr, "没有找到微信进程 请先启动微信", "错误", 0);
		return FALSE;
	}
	//检测dll是否已经注入
	if (CheckIsInject(dwPid))
	{
		...
		if (hProcess == nullptr)
		{
			MessageBoxA(nullptr, "进程打开失败", "错误", 0);
			return FALSE;
		}
		//在微信进程中申请内存
		LPVOID pAddress = VirtualAllocEx(hProcess, nullptr, MAX_PATH, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
		if (pAddress == nullptr)
		{
			MessageBoxA(nullptr, "内存分配失败", "错误", 0);
			return FALSE;
		}
		//写入dll路径到微信进程
		DWORD dwWrite = 0;
		...
		{
			MessageBoxA(nullptr, "路径写入失败", "错误", 0);
			return FALSE;
		}

		//获取LoadLibraryA函数地址
		FARPROC pLoadLibraryAddress = GetProcAddress(GetModuleHandleA("kernel32.dll"), "LoadLibraryA");
		if (pLoadLibraryAddress == nullptr)
		{
			MessageBoxA(nullptr, "获取LoadLibraryA函数地址失败", "错误", 0);
			return FALSE;
		}
		//远程线程注入dll
		HANDLE hRemoteThread = CreateRemoteThread(hProcess, nullptr, 0, (LPTHREAD_START_ROUTINE)pLoadLibraryAddress,
		                                          pAddress, 0, nullptr);
		if (hRemoteThread == nullptr)
		{
			MessageBoxA(nullptr, "远程线程注入失败", "错误", 0);
			return FALSE;
		}
	...
	{
		MessageBoxA(nullptr, "dll已经注入，请勿重复注入", "提示", 0);
		return FALSE;
	}
...
BOOL CheckIsInject(DWORD dwProcessid)
{
	...
	{
		MessageBoxA(nullptr, "创建模块快照失败", "错误", MB_OK);
		return FALSE;
	}
	//通过模块快照句柄获取第一个模块的信息
	if (!Module32First(hModuleSnap, &me32))
	{
		MessageBoxA(nullptr, "获取第一个模块的信息失败", "错误", MB_OK);
		//获取失败则关闭句柄
...
void UnloadDll()
{
	...
	if (dwPid == 0)
	{
		MessageBoxA(nullptr, "没有找到微信进程或者微信没有启动", "错误", 0);
		return;
	}
	...
	if (flag == FALSE)
	{
		MessageBoxA(nullptr, "找不到目标模块", "错误", 0);
		return;
	}
	...
	//创建远程线程执行FreeLibrary
	HANDLE hThread = CreateRemoteThread(hPro, nullptr, 0, (LPTHREAD_START_ROUTINE)pFun, ME32.modBaseAddr, 0, nullptr);
	if (!hThread)
	{
		MessageBoxA(nullptr, "创建远程线程失败", "错误", 0);
		return;
	}
	...
	CloseHandle(hThread);
	CloseHandle(hPro);
	MessageBoxA(nullptr, "dll卸载成功", "Tip", 0);
}
