...
void CDecryptImage::OnBnClickedDecryptimage()
{
	...
	//打开文件
	HANDLE hFile = CreateFileW(m_ImagePath, GENERIC_ALL, NULL, nullptr, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
	if (hFile == INVALID_HANDLE_VALUE)
	{
		MessageBoxA(nullptr, "打开文件失败", "错误", 0);
		return;
	}
	...
	//读取文件到内存
	if (ReadFile(hFile, lpBuff, dwSize, nullptr, nullptr) == 0)
	{
		MessageBoxA(nullptr, "读取文件失败", "错误", 0);
		return;
	}
	UpdateData(TRUE);
	...
	//打开文件
	HANDLE hNewFile = CreateFileW(NewFilePath, GENERIC_ALL, NULL, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL,
	                              nullptr);
	if (hNewFile == INVALID_HANDLE_VALUE)
	{
		MessageBoxA(nullptr, "创建文件失败", "错误", 0);
		return;
	}

	WriteFile(hNewFile, lpBuff, dwSize, nullptr, nullptr);
	CloseHandle(hNewFile);

	MessageBoxA(nullptr, "解密成功 请在同目录下查看", "Tip", MB_OK);
